...
class SettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->hasFile('path_logo')) {       //Memeriksa apakah file dengan nama input 'path_logo' ada dalam request.
            $file = $request->file('path_logo');
            $nama = 'logo-' . date('YmdHis') . '.' . $file->getClientOriginalExtension();
            $file->move(public_path('/img'), $nama);    //Memindahkan file ke direktori tujuan di server (/img di dalam direktori publik).
