...
<script>
    ...
    let stok = $(this).data('stok');  // Mendapatkan stok dari atribut data-stok
    ...
    if (jumlah > 10000) {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Gagal',
            text: 'Jumlah tidak boleh lebih dari 10.000',
        });
        return;
    }
    ...
    let jumlah = $('#jumlah_produk').val();  // Ambil nilai jumlah dari input
    let id_produk = $('#id_produk').val();  // Ambil ID produk dari input
    
    $.get(`{{ url('/produk') }}/${id_produk}`, function(produk) {
